...
class TestMain(unittest.TestCase):
    ...
    def test_json_input_does_exists(self):
        with capture(empiar_depositor_main, ["ABC123", "tests/deposition_json/working_example.json", ""]) as output:
            self.assertNotEqual(output, 'The specified JSON file does not exist\n')

    def test_ascp_does_not_exist(self):
        with capture(empiar_depositor_main, ["ABC123", "tests/deposition_json/working_example.json", "", "-aascp",
                                             "-ppassword"]) as output:
            self.assertEqual(output, "The specified Aspera executable does not exist\n")

    def test_wrong_aspera_executable(self):
        with capture(empiar_depositor_main, ["ABC123", "tests/deposition_json/working_example.json",
                                             "-atests/img/entry_thumbnail.gif", "img/entry_thumbnail.gif",
                                             "-ppassword"]) as output:
